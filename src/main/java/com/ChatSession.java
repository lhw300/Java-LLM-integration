	...
	public void addUserHis(String text) {
	    ...
	    public ChatAnswer ask(String text) {
	        ...
	        try {
	            ...
	            /*
	            // 构建只包含历史问题和知识库的字符串上下文
	            StringBuilder queryHistoryBuilder = new StringBuilder();
	            for (QueryHistory.Message msg : queryHistory.getMessages()) {
	            ...
	            */
	            ...
	            if (!historyContextStr.trim().isEmpty() && rewrite_prompt != null && !rewrite_prompt.isEmpty()) {
	                ...
	                String rewritten = SessionManager.rewriteQuery(text, historyContextStr, rewrite_prompt);
	            ...
	            List<SearchService.KnowledgeItem> items = SearchService.getRelevantKnowledge(optimizedQuery);
	         ...
	         // 分别构建【完整知识】和【摘要知识】
	            ...
	            for (int i = 0; i < items.size(); i++) {
	                ...
	                // 1. 完整知识 (发给大模型做最终回答用)
	                fullContextBuilder.append(String.format("%d. %s\n", i + 1, item.content));
	                
	                // 2. 摘要知识 (专门提取【】里的内容)
	                String summary = extractSummary(item.content);
		...
		try {
			...
			if (bestDistance > SIMILARITY_THRESHOLD) {
				...
				for (int i = 0; i < Math.min(3, items.size()); i++) {
					System.out.println((i + 1) + ". " + items.get(i).content + " (距离: "
							+ String.format("%.4f", items.get(i).distance) + ")");
				}
				System.out.println();
