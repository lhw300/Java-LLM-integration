        ...
        config.setMaxLifetime(1800000);
	     ...
	     // 🌟 规范修改：方法签名增加 EmbeddingClient 参数
	        public static List<KnowledgeItem> getRelevantKnowledge(String query, EmbeddingClient embedClient) throws Exception {
	            // 使用外部传入的接口将文字转为向量
			  ...
			  public static List<KnowledgeItem> getRelevantKnowledge(String query) throws
			  Exception { double[] vector = getEmbedding(query); return
			  searchTopKnowledge(vector, 3); }
			...
			private static double[] getEmbedding(String text) throws Exception {
			  ObjectNode root = MAPPER.createObjectNode(); root.put("model",
			  "text-embedding-3-small"); // 对应 1536 维度 root.put("input", text);
			  
			  RequestBody body = RequestBody.create(MAPPER.writeValueAsString(root),
			  MediaType.parse("application/json")); Request request = new Request.Builder()
			  .url("https://api.openai.com/v1/embeddings") .header("Authorization",
			  "Bearer " + OPENAI_API_KEY) .post(body).build();
			  
			  try (Response response = CLIENT.newCall(request).execute()) { JsonNode
			  resJson = MAPPER.readTree(response.body().string()); JsonNode embeddingNode =
			  resJson.path("data").get(0).path("embedding"); double[] vector = new
			  double[embeddingNode.size()]; for (int i = 0; i < embeddingNode.size(); i++)
			  { vector[i] = embeddingNode.get(i).asDouble(); } return vector; } }
